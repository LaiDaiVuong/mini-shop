            ...
            <div style={{ position: 'relative' }}>
              ...
              {showUserDropdown && (
                <div style={{
                  ...
                  {user ? (
                    <>
                      <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: 2 }}>{user.fullname}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--color-accent)', marginBottom: 10 }}>
                        {user.role === 'admin' ? '👑 Quản Trị Viên VIP' : '✨ Khách Hàng VIP'}
                      </div>
                      <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {user.role === 'admin' && (
                          <Link href="/admin" className="dropdown-link" style={{ fontSize: '0.825rem', fontWeight: 600, color: 'var(--color-accent)' }}>
                            ⚡ Trang Quan Ly Admin
                          </Link>
                        )}
                        <button 
                          onClick={() => { logout(); setShowUserDropdown(false); }}
                          style={{ textAlign: 'left', fontSize: '0.825rem', color: '#ef4444', fontWeight: 600 }}
                        >
                          Đăng Xuất
                        </button>
                      </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: 4 }}>Tài khoản giả lập</div>
                      <Link href="/login" onClick={() => setShowUserDropdown(false)} style={{ display: 'block', padding: '8px 12px', background: 'var(--color-accent)', color: '#fff', textAlign: 'center', borderRadius: 6, fontWeight: 700, fontSize: '0.8rem' }}>
                        Đăng Nhập
                      </Link>
                    </div>
                  )}
        ...
        <>
          ...
          <div className="mobile-nav-drawer active">
            ...
            <div className="mobile-nav-links" onClick={() => setIsMobileOpen(false)}>
              ...
              <Link href="/admin">Trang Quản Lý Admin</Link>
            </div>
          </div>
        </>
      )}
