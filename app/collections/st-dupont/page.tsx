...
export default function STDupontCollectionPage() {
  ...
  let filtered = INITIAL_PRODUCTS_DATA.filter(p => p.category === 'st-dupont');
      ...
      <section className="collection-hero">
        <div className="container">
          <div className="breadcrumb" style={{ marginBottom: 16 }}>
            ...
            <span style={{ color: '#64748b', margin: '0 8px' }}>&rsaquo;</span>
            <Link href="/products" style={{ color: '#cbd5e1' }}>Bộ Sưu Tập</Link>
            <span style={{ color: '#64748b', margin: '0 8px' }}>&rsaquo;</span>
        ...
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, marginBottom: 24, padding: '16px 20px', background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0' }}>
            <div style={{ fontWeight: 700, fontSize: '1.05rem', color: 'var(--color-text-main)' }}>
              Danh sách sản phẩm S.T. Dupont ({filtered.length})
            </div>

            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <input 
                type="text" 
                placeholder="Tìm theo tên sản phẩm S.T. Dupont..." 
                value={search}
