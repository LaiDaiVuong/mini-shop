...
export default function RowentaCollectionPage() {
  const [productsList, setProductsList] = useState<Product[]>(
    INITIAL_PRODUCTS_DATA.filter(p => p.category === 'rowenta')
  );

  useEffect(() => {
    fetchProductsFromSupabase().then(data => {
      if (data && data.length > 0) {
        setProductsList(data.filter(p => p.category === 'rowenta'));
      }
    });
  }, []);
      ...
      <section className="collection-hero">
        <div className="container">
          <div className="breadcrumb" style={{ marginBottom: 16 }}>
            ...
            <Link href="/products" style={{ color: '#fef3c7' }}>Sản Phẩm</Link>
            <span style={{ color: 'rgba(255,255,255,0.4)' }}>&rsaquo;</span>
        ...
        <div className="container">
          <div className="products-top-bar" style={{ marginBottom: 30 }}>
            <div className="products-result-count">
              Hiển thị <strong>{productsList.length}</strong> tuyệt tác Rowenta R10 cổ điển từ CHLB Đức
            </div>
          </div>
